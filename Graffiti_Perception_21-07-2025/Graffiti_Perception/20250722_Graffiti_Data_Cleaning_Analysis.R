...
gender_table <- prop.table(table(df_clean$gender)) * 100
housing_table <- prop.table(table(df_clean$housing)) * 100


# NOTE : This is just the sample table, you have to include more varibles in this table.
...
summary_table <- tibble(
  Variable = c(
    "Age", 
    "Gender", 
    "\tMan", "\tVrouw", "\tNon-binair", "\tAnders:", "\tWil ik liever niet zeggen",
    "Housing",
    "\tEigenaar", "\tHuurder", "\tKot", "\tAnders:",
    "Perception score", 
    "Reporting score"
  ),
  Min = round(c(
  ...
  Percentage = round(c(
    ...
    gender_table["Man"],
    gender_table["Vrouw"],
    gender_table["Non-binair"],
    gender_table["Anders:"],
    gender_table["Wil ik liever niet zeggen"],
    NA,
    housing_table["Eigenaar van een (koop)huis/appartement"],
    housing_table["Ik huur een woning/appartement"],
    housing_table["Ik huur een kot"],
    housing_table["Anders:"],
    NA, NA
  ), 2)
)
...
df_clean <- df_clean |>
  ...
  rowwise() |>
  mutate(
    # Sum scores for graffiti perception (higher = more negative perception)
    graffiti_perception_sum = sum(c_across(all_of(likert_vars_q9)), na.rm = TRUE),
    
    # Sum scores for graffiti reporting (higher = more likely to report)
    graffiti_reporting_sum  = sum(c_across(all_of(likert_vars_q10)), na.rm = TRUE)
  ) |>
  ungroup()
