...
# ===============================================================================
# SECTION 3.5: TRANSLATION FUNCTION FOR DUTCH TO ENGLISH
# ===============================================================================

# Function to translate all Dutch labels and categories to English
translate_data_to_english <- function(df) {
  
  # Create translation dictionaries
  
  # Gender translations (shortened)
  gender_labels <- c(
    "Man" = "Male",
    "Vrouw" = "Female", 
    "Non-binair" = "Non-binary",
    "Anders:" = "Other",
    "Wil ik liever niet zeggen" = "No answer"
  )
  
  # Education translations (shortened)
  education_labels <- c(
    "Diploma Lager Onderwijs" = "Primary",
    "Diploma Middelbaar" = "Secondary", 
    "Diploma Hoger Onderwijs" = "Higher",
    "Wil ik liever niet zeggen" = "No answer"
  )
  
  # Housing translations (shortened)
  housing_labels <- c(
    "Eigenaar van een (koop)huis/appartement" = "Owner",
    "Ik huur een woning/appartement" = "Renter",
    "Ik huur een kot" = "Student room",
    "Anders:" = "Other"
  )
  
  # Work situation translations (shortened)
  work_labels <- c(
    "Ik werk voltijds" = "Full-time",
    "Ik werk parttime" = "Part-time", 
    "Ik ben momenteel werkloos" = "Unemployed",
    "Ik ben zelfstandige" = "Self-employed",
    "Ik ben student" = "Student",
    "Ik ben gepensioneerd" = "Retired",
    "Anders:" = "Other"
  )
  
  # Graffiti amount translations (shortened)
  graffiti_amount_labels <- c(
    "Minder dan 5" = "<5",
    "Tussen 5 en 10" = "5-10",
    "Tussen 10 en 15" = "10-15", 
    "Tussen 15 en 20" = "15-20",
    "Meer dan 20" = ">20"
  )
  
  # Likert scale translations (shortened for perception questions)
  likert_perception_labels <- c(
  ...
  # Likert scale translations (shortened for reporting likelihood)
  likert_reporting_labels <- c(
    "Helemaal niet waarschijnlijk" = "Very unlikely",
    "Niet waarschijnlijk" = "Unlikely",
    "Neutraal" = "Neutral", 
    "Waarschijnlijk" = "Likely",
    "Heel waarschijnlijk" = "Very likely"
  )
  
  # Previous reporting translations (shortened)
  previous_reporting_labels <- c(
    "Ja, ik heb het gerapporteerd aan de politie" = "Police",
    "Ja, ik heb een melding gemaakt bij Stad Gent" = "City of Ghent",
    "Ja, ik heb een melding gemaakt bij de verhuurder" = "Landlord",
    "Neen, ik heb geen graffiti gerapporteerd aan één van de bevoegde instanties" = "No report"
  )
  ...
  # Variable label translations (shortened)
  variable_label_translations <- list(
    "Q4" = "Sector",
    "QID1" = "Birth year",
    "Q2" = "Gender",
    "Q6" = "Education",
    ...
    "Q3" = "Work situation",
    "Q8" = "Graffiti amount",
    "Q9_1" = "Graffiti always illegal",
    "Q9_2" = "Graffiti is nuisance",
    "Q9_3" = "Graffiti is art",
    "Q9_4" = "Graffiti is vandalism",
    "Q9_6" = "Graffiti damages buildings",
    "Q9_7" = "Graffiti looks messy",
    "Q10_1" = "Street art always illegal",
    "Q10_2" = "Street art is nuisance",
    "Q10_3" = "Street art is art",
    "Q10_4" = "Street art is vandalism",
    "Q10_6" = "Street art damages buildings",
    "Q10_7" = "Street art looks messy",
    "Q18_1" = "Report tags",
    "Q18_2" = "Report throw-ups",
    "Q18_3" = "Report masterpieces",
    "Q18_4" = "Report slogans",
    "Q18_5" = "Report illustrations",
    "Q18_6" = "Report street art",
    "Q11" = "Previous reporting",
    "Q19" = "City removal policy",
    "Q21" = "Pay for removal"
  )
  ...
  # Translate variable labels (attributes)
  for (var_name in names(variable_label_translations)) {
    if (var_name %in% names(df_translated)) {
      attr(df_translated[[var_name]], "label") <- variable_label_translations[[var_name]]
    }
  }
  
  # Translate factor labels for each variable
  # Gender (Q2)
  if ("Q2" %in% names(df_translated) && haven::is.labelled(df_translated$Q2)) {
    attr(df_translated$Q2, "labels") <- gender_labels[names(attr(df_translated$Q2, "labels"))]
  }
  
  # Education (Q6)
  if ("Q6" %in% names(df_translated) && haven::is.labelled(df_translated$Q6)) {
    attr(df_translated$Q6, "labels") <- education_labels[names(attr(df_translated$Q6, "labels"))]
  }
  
  # Housing (Q7)
  if ("Q7" %in% names(df_translated) && haven::is.labelled(df_translated$Q7)) {
    attr(df_translated$Q7, "labels") <- housing_labels[names(attr(df_translated$Q7, "labels"))]
  }
  
  # Work situation (Q3)
  if ("Q3" %in% names(df_translated) && haven::is.labelled(df_translated$Q3)) {
    attr(df_translated$Q3, "labels") <- work_labels[names(attr(df_translated$Q3, "labels"))]
  }
  
  # Graffiti amount (Q8)
  if ("Q8" %in% names(df_translated) && haven::is.labelled(df_translated$Q8)) {
    attr(df_translated$Q8, "labels") <- graffiti_amount_labels[names(attr(df_translated$Q8, "labels"))]
  }
  
  # Perception questions (Q9_* and Q10_*
  perception_vars <- paste0("Q9_", c(1, 2, 3, 4, 6, 7))
  streetart_vars <- paste0("Q10_", c(1, 2, 3, 4, 6, 7))
  
  for (var in c(perception_vars, streetart_vars)) {
    if (var %in% names(df_translated) && haven::is.labelled(df_translated[[var]])) {
      current_labels <- names(attr(df_translated[[var]], "labels"))
      if (!is.null(current_labels)) {
        attr(df_translated[[var]], "labels") <- likert_perception_labels[current_labels]
      }
    }
  }
  
  # Reporting likelihood questions (Q18_*
  reporting_vars <- paste0("Q18_", c(1, 2, 3, 4, 5, 6))
  
  for (var in reporting_vars) {
    if (var %in% names(df_translated) && haven::is.labelled(df_translated[[var]])) {
      current_labels <- names(attr(df_translated[[var]], "labels"))
      if (!is.null(current_labels)) {
        attr(df_translated[[var]], "labels") <- likert_reporting_labels[current_labels]
      }
    }
  }
  
  # Previous reporting (Q11)
  if ("Q11" %in% names(df_translated) && haven::is.labelled(df_translated$Q11)) {
    attr(df_translated$Q11, "labels") <- previous_reporting_labels[names(attr(df_translated$Q11, "labels"))]
  }
  
  # Yes/No questions (Q19, Q21)
  yes_no_vars <- c("Q19", "Q21")
  for (var in yes_no_vars) {
    if (var %in% names(df_translated) && haven::is.labelled(df_translated[[var]])) {
      attr(df_translated[[var]], "labels") <- yes_no_labels[names(attr(df_translated[[var]], "labels"))]
    }
  }
...
# Translate Dutch labels and categories to English
df_complete <- translate_data_to_english(df_complete)
...
df_clean <- df_complete |>
  mutate(
    ...
    # NOTE TESSA: zones cannot be changed to numeric because they start with "a", so i disabled this line of code
    # Q4 = as.numeric(Q4)
  ) |>
  rename(
...
# First convert labelled variables to factors to get English labels
df_clean_factors <- df_clean |>
  mutate(
    gender_factor = haven::as_factor(gender),
    housing_factor = haven::as_factor(housing)
  )

gender_table <- prop.table(table(df_clean_factors$gender_factor)) * 100
housing_table <- prop.table(table(df_clean_factors$housing_factor)) * 100

# NOTE : This is just the sample table, you can include more variables in this table.
# Build comprehensive summary table with shortened labels
# This table will be exported to Word for your report
summary_table <- tibble(
  Variable = c(
    "Age", 
    "Gender", 
    "\tMale", "\tFemale", "\tNon-binary", "\tOther", "\tNo answer",
    "Housing",
    "\tOwner", "\tRenter", "\tStudent room", "\tOther",
    "Perception score", 
    "Reporting score"
  ),
  Min = round(c(
  ...
  Percentage = round(c(
    ...
    gender_table["Male"],
    gender_table["Female"],
    gender_table["Non-binary"],
    gender_table["Other"],
    gender_table["No answer"],
    NA,
    housing_table["Owner"],
    housing_table["Renter"],
    housing_table["Student room"],
    housing_table["Other"],
    NA, NA
  ), 2)
)
...
bold_rows <- which(summary_table$Variable %in% c("Age", "Gender", "Housing", "Perception score", "Reporting score"))
...
likert_vars_q10 <- paste0("Q18_", c(1, 2, 3, 4, 5, 6))   # Graffiti reporting items

# Identify which variables are labelled (from SPSS) but NOT our Likert items
all_likert_vars <- c(likert_vars_q9, likert_vars_q10)
...
# ===============================================================================
# SECTION 6.5: FACTOR LEVEL MANAGEMENT FOR MODEL INTERPRETATION
# ===============================================================================
# Set meaningful factor levels and reference categories for better model interpretation

df_clean <- df_clean |>
  mutate(
    # Set factor levels for gender with Male as reference
    gender = factor(gender, levels = c("Male", "Female", "Non-binary", "Other", "No answer")),
    
    # Set factor levels for education with Primary as reference  
    education = factor(education, levels = c("Primary", "Secondary", "Higher", "No answer")),
    
    # Set factor levels for housing with Owner as reference
    housing = factor(housing, levels = c("Owner", "Renter", "Student room", "Other")),
    
    # Create a work situation factor if Q3 exists (optional)
    work_situation = if("Q3" %in% names(.)) {
      factor(haven::as_factor(Q3), 
             levels = c("Full-time", "Part-time", "Unemployed", "Self-employed", "Student", "Retired", "Other"))
    } else {
      NULL
    }
  )

# Set contrast coding for factors (treatment contrasts with meaningful reference levels)
contrasts(df_clean$gender) <- contr.treatment(nlevels(df_clean$gender), base = 1)  # Male as reference
contrasts(df_clean$education) <- contr.treatment(nlevels(df_clean$education), base = 1)  # Primary as reference
contrasts(df_clean$housing) <- contr.treatment(nlevels(df_clean$housing), base = 1)  # Owner as reference

# Optional: Set work situation contrasts if the variable exists
if (!is.null(df_clean$work_situation)) {
  contrasts(df_clean$work_situation) <- contr.treatment(nlevels(df_clean$work_situation), base = 1)  # Full-time as reference
}
# ===============================================================================
...
model_perception <- lmer(
  graffiti_perception_sum ~ age + gender + education + housing + (1 | sector),
  data = df_clean
)
...
# Function to clean predictor names for better readability
clean_predictor_names <- function(names) {
  names %>%
    # Clean gender categories
    stringr::str_replace("genderFemale", "Gender: Female") %>%
    stringr::str_replace("genderNon-binary", "Gender: Non-binary") %>%
    stringr::str_replace("genderOther", "Gender: Other") %>%
    stringr::str_replace("genderNo answer", "Gender: No answer") %>%
    
    # Clean education categories  
    stringr::str_replace("educationSecondary", "Education: Secondary") %>%
    stringr::str_replace("educationHigher", "Education: Higher") %>%
    stringr::str_replace("educationNo answer", "Education: No answer") %>%
    
    # Clean housing categories
    stringr::str_replace("housingRenter", "Housing: Renter") %>%
    stringr::str_replace("housingStudent room", "Housing: Student room") %>%
    stringr::str_replace("housingOther", "Housing: Other") %>%
    
    # Clean work situation categories (if included)
    stringr::str_replace("work_situationPart-time", "Work: Part-time") %>%
    stringr::str_replace("work_situationUnemployed", "Work: Unemployed") %>%
    stringr::str_replace("work_situationSelf-employed", "Work: Self-employed") %>%
    stringr::str_replace("work_situationStudent", "Work: Student") %>%
    stringr::str_replace("work_situationRetired", "Work: Retired") %>%
    stringr::str_replace("work_situationOther", "Work: Other") %>%
    
    # Clean other common terms
...
# Clean predictor names for better readability
fixed_df$Predictor <- clean_predictor_names(fixed_df$Predictor)
...
#higher scores reflect more negative perceptions of graffiti. Older participants reported 
...
perception_plot <- ggplot(ranefs_perception, aes(x = factor(sector, levels = 1:30), y = intercept, fill = effect_category)) +
  geom_col(width = 0.7) +
  ...
  labs(
    ...
    subtitle = "Positive values indicate more negative perception than average"
  ) +
...
vif_values <- vif(lm(graffiti_perception_sum ~ age + gender + education + housing, data = df_clean))
...
model_reporting <- lmer(
  graffiti_reporting_sum ~ age + gender + education + housing + (1 | sector),
  data = df_clean
)
...
if (sector_variance < 0.01) {
  ...
  model_reporting_fixed <- lm(
    graffiti_reporting_sum ~ age + gender + education + housing,
    data = df_clean
  )
  ...
  reporting_plot <- ggplot(ranefs_reporting, aes(x = factor(sector, levels = 1:30), y = intercept, fill = effect_category)) +
    geom_col(width = 0.7) +
    scale_fill_manual(values = effect_colors) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    labs(
...
# 4. Multicollinearity check
vif_values_reporting <- vif(lm(graffiti_reporting_sum ~ age + gender + education + housing, data = df_clean))
...
# Create a simple map of the study area with sector codes displayed
map_plot <- ggplot(sf_gent_filtered) +
  geom_sf(fill = "lightblue", color = "white", size = 0.5) +
  geom_sf_text(aes(label = sectorcode), size = 3, color = "black") +
  labs(
...
# Clean predictor names for better readability
perception_fixed_df$Predictor <- clean_predictor_names(perception_fixed_df$Predictor)
...
# Clean predictor names for better readability
reporting_fixed_df$Predictor <- clean_predictor_names(reporting_fixed_df$Predictor)
...
cor_vars_rep <- c("age", "graffiti_reporting_sum", "gender", "education", "housing")
