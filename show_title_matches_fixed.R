...
# Add index columns
master_data$master_idx <- 1:nrow(master_data)
clean_data$clean_idx <- 1:nrow(clean_data)
...
exact_matches <- inner_join(
  master_data %>% select(Title_of_the_study, normalized_title, master_idx),
  clean_data %>% select(Title, normalized_title, clean_idx),
  by = "normalized_title"
)
...
for (i in 1:nrow(unmatched_master)) {
  ...
  if (length(matches) > 0) {
    ...
    match_idx <- matches[1]
    title_matches <- rbind(title_matches, data.frame(
      Master_Title = unmatched_master$Title_of_the_study[i],
...
cat("Found", fuzzy_count, "fuzzy matches (master in clean)\n")

# 3. Reverse fuzzy matching - clean title contained in master title
cat("Step 3: Reverse fuzzy matching (clean in master)...\n")
unmatched_master <- master_data[!master_data$master_idx %in% matched_master, ]
unmatched_clean <- clean_data[!clean_data$clean_idx %in% matched_clean, ]

reverse_fuzzy_count <- 0
for (i in 1:nrow(unmatched_clean)) {
  clean_title <- unmatched_clean$normalized_title[i]
  
  # Find master titles that contain the clean title
  matches <- which(str_detect(unmatched_master$normalized_title, fixed(clean_title)))
  
  if (length(matches) > 0) {
    # Take the first match
    match_idx <- matches[1]
    title_matches <- rbind(title_matches, data.frame(
      Master_Title = unmatched_master$Title_of_the_study[match_idx],
      Clean_Title = unmatched_clean$Title[i],
      Match_Type = "Reverse Fuzzy (Clean in Master)",
      stringsAsFactors = FALSE
    ))
    matched_master <- c(matched_master, unmatched_master$master_idx[match_idx])
    matched_clean <- c(matched_clean, unmatched_clean$clean_idx[i])
    reverse_fuzzy_count <- reverse_fuzzy_count + 1
  }
}
cat("Found", reverse_fuzzy_count, "reverse fuzzy matches\n")

# 4. Word overlap matching
cat("Step 4: Word overlap matching...\n")
unmatched_master <- master_data[!master_data$master_idx %in% matched_master, ]
unmatched_clean <- clean_data[!clean_data$clean_idx %in% matched_clean, ]
...
for (i in 1:nrow(unmatched_master)) {
  ...
  if (!is.null(best_match_idx)) {
    ...
    word_overlap_count <- word_overlap_count + 1
  }
}
cat("Found", word_overlap_count, "word overlap matches\n")

# 5. Special pattern matching (AU/NL/UK)
cat("Step 5: Special pattern matching...\n")
unmatched_master <- master_data[!master_data$master_idx %in% matched_master, ]
unmatched_clean <- clean_data[!clean_data$clean_idx %in% matched_clean, ]
...
for (i in 1:nrow(unmatched_master)) {
  ...
  # Check if master title contains AU/NL/UK pattern
  if (str_detect(master_title, "(au|nl|uk)")) {
    # Extract the part before the pattern
    base_title <- str_extract(master_title, "^.*?(?=\\s+(au|nl|uk))")
    
    if (!is.na(base_title) && nchar(str_trim(base_title)) > 10) {
      # Look for clean titles that match this base
      matches <- which(str_detect(unmatched_clean$normalized_title, fixed(str_trim(base_title))))
...
cat("Found", special_count, "special pattern matches\n")

# Display results
cat("\n=== TITLE MATCHING RESULTS ===\n")
cat("Total matches found:", nrow(title_matches), "\n")
cat("Master dataset size:", nrow(master_data), "\n")
cat("Clean dataset size:", nrow(clean_data), "\n")
cat("Coverage:", round(nrow(title_matches) / nrow(master_data) * 100, 1), "%\n\n")

# Display match type summary
match_summary <- title_matches %>% 
  group_by(Match_Type) %>% 
...
for (i in 1:nrow(title_matches)) {
  cat("\n", i, ". Match Type:", title_matches$Match_Type[i], "\n")
  cat("   Master: ", title_matches$Master_Title[i], "\n")
  cat("   Clean:  ", title_matches$Clean_Title[i], "\n")
}

# Save results
write.csv(title_matches, "20250709_Analysis & Results/title_matches.csv", row.names = FALSE)
cat("\nResults saved to: 20250709_Analysis & Results/title_matches.csv\n")
