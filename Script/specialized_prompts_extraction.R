# Comprehensive Extraction Based on 19 Specialized Prompts
# Maps each field from the specialized extraction prompts to individual columns
...
cat("Starting comprehensive extraction based on 19 specialized prompts...\n")
...
# Function to extract numbered list items (for variables)
...
# For variable extraction, we need to collect all variables first, then create columns
all_demo_vars <- list()
all_econ_vars <- list()
all_env_vars <- list()
all_dist_vars <- list()
...
# Create columns for unique variables
...
# Add variable columns and fill them
...
# Save the result
write_csv(result_data, "Output/specialized_prompts_extracted_dataset.csv")

cat("\n=== EXTRACTION SUMMARY ===\n")
cat("Original dataset:", nrow(df), "rows x", ncol(df), "columns\n")
cat("Extracted dataset:", nrow(result_data), "rows x", ncol(result_data), "columns\n")
cat("Output saved to: Output/specialized_prompts_extracted_dataset.csv\n")
...
cat("\nExtraction based on 19 specialized prompts completed!\n")
