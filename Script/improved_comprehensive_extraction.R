# IMPROVED COMPREHENSIVE GROUPED EXTRACTION - ALL DATA AND VARIABLES
# Fixed regex patterns to properly extract quoted text and all field content
...
cat("Starting IMPROVED COMPREHENSIVE grouped extraction with ALL data...\n")
cat("Original dataset:", nrow(df), "rows x", ncol(df), "columns\n")

# IMPROVED function to extract ALL field information from text with better regex patterns
extract_all_fields_improved <- function(text, field_names) {
  ...
  for (field_name in field_names) {
    # IMPROVED patterns that handle quotes, hyphens, and complex text properly
    patterns <- c(
      # Pattern for double quoted text (highest priority)
      paste0("\\*\\*", field_name, ":\\*\\*\\s*\"([^\"]+)\""),
      paste0("- \\*\\*", field_name, ":\\*\\*\\s*\"([^\"]+)\""),
      
      # Pattern for text until next field or end - NO character restrictions
      paste0("\\*\\*", field_name, ":\\*\\*\\s*(.+?)(?=\\s*\\n\\s*-\\s*\\*\\*|\\s*\\n\\s*\\*\\*|$)"),
      paste0("- \\*\\*", field_name, ":\\*\\*\\s*(.+?)(?=\\s*\\n\\s*-\\s*\\*\\*|\\s*\\n\\s*\\*\\*|$)"),
      
      # Fallback patterns for any remaining content
      paste0("\\*\\*", field_name, ":\\*\\*\\s*(.+?)(?=\\n|$)"),
      paste0("- ", field_name, ":\\s*(.+?)(?=\\s*\\n\\s*-|$)"),
      paste0(field_name, ":\\s*(.+?)(?=\\s*\\n\\s*-|$)")
    )
    ...
    for (pattern in patterns) {
      ...
      if (!is.na(match[1, 2])) {
        ...
        # Remove only completely empty results or standard "not mentioned" phrases
        if (result != "" && 
...
# Extract ALL core fields comprehensively with IMPROVED patterns
cat("Extracting ALL core fields with IMPROVED patterns...\n")

for (i in 1:nrow(df)) {
  if (i %% 10 == 0) cat("Processing study", i, "of", nrow(df), "\n")
  ...
  # 3. SPATIAL UNITS - Extract ALL fields including rationale (IMPROVED)
  ...
  if (!is.na(text)) {
    theory_fields <- extract_all_fields_improved(text, c("Theoretical Framework", "Research Objectives", "Literature Gap", 
                                                "Study Motivation"))
  ...
  if (!is.na(text)) {
    method_fields <- extract_all_fields_improved(text, c("Study Design", "Discrete Choice Model", "Model Specification", 
  ...
  if (!is.na(text)) {
    data_fields <- extract_all_fields_improved(text, c("Data Cleaning", "Variable Construction", "Missing Data Handling", 
  ...
  if (!is.na(text)) {
    performance_fields <- extract_all_fields_improved(text, c("Model Performance", "Information Criteria", "Goodness-of-Fit Tests", 
  ...
  if (!is.na(text)) {
    results_fields <- extract_all_fields_improved(text, c("Main Results", "Significant Predictors", "Effect Directions", 
  ...
  if (!is.na(text)) {
    scale_fields <- extract_all_fields_improved(text, c("Scale Effects", "Scale Sensitivity", "Spatial Autocorrelation", 
  ...
  if (!is.na(text)) {
    limitations_fields <- extract_all_fields_improved(text, c("Data Quality Issues", "Missing Data", "Data Source Limitations", 
  ...
  if (!is.na(text)) {
    implications_fields <- extract_all_fields_improved(text, c("Theoretical Contributions", "Policy Implications", "Crime Prevention Implications",
...
# Save the IMPROVED comprehensive result
write_csv(result_data, "Output/improved_comprehensive_extraction.csv")

# Generate comprehensive summary report
cat("\n=== IMPROVED COMPREHENSIVE EXTRACTION SUMMARY ===\n")
cat("Original dataset:", nrow(df), "rows x", ncol(df), "columns\n")
cat("IMPROVED dataset:", nrow(result_data), "rows x", ncol(result_data), "columns\n")
cat("Output saved to: Output/improved_comprehensive_extraction.csv\n")

# Show extraction success rates for key fields
cat("\n=== IMPROVED FIELD EXTRACTION SUMMARY ===\n")
...
cat("\nIMPROVED COMPREHENSIVE extraction completed!\n")
cat("ALL data fields and variables extracted with IMPROVED regex patterns.\n")
cat("Fixed issues with quoted text, hyphens, and complex punctuation.\n")
